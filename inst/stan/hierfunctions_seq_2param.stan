...
matrix get_morehierparam_sigmawpriorsd(matrix a_sigma_fixed, matrix a_sigma_estimate,
        real prior_sd_a, // prior sd for intercept (repeated across the parameters)
        int a_n_sigma, int k, //number of parameters, here k = num_basis - num_constrained_zero,
                  int a_n_sigma_fixed,  int a_n_sigma_estimate
                  ){
  matrix[a_n_sigma +1, k] a_sigma;
  ...
  if (a_n_sigma_estimate > 0) {
      a_sigma[(1 + a_n_sigma_fixed + 1):(1+a_n_sigma), ] = a_sigma_estimate;
  }
  return(a_sigma);
}
...
// matrix of non-zero coefficients
matrix get_morehierparam_star(matrix a_sigma,    matrix a_raw_fixed, matrix a_raw_estimate,
              int k,  //number of parameters, here k = num_basis - num_constrained_zero,
                 int a_n_terms_fixed, int a_n_terms_estimate,
                  array[] int a_re_start, array[] int a_re_end,
                  int C, int a_n_terms,
...
/////////////////////////////////////////////////////
data {
  int k; // number of parameters in vector mu
  ...
  // for mu hierarchical set up (indep of k)
  ...
  //moreparam mu
  matrix[mu_n_terms_fixed, k] mu_raw_fixed;
  matrix<lower=0>[mu_n_sigma_fixed, k] mu_sigma_fixed;
...
parameters {
  ...
  // moreparam mu
  matrix[mu_n_terms_estimate, k] mu_raw_estimate;
  matrix<lower=0>[mu_n_sigma_estimate, k] mu_sigma_estimate;
...
transformed parameters {
  matrix[mu_n_sigma + 1, k] mu_sigma = get_morehierparam_sigmawpriorsd(mu_sigma_fixed, mu_sigma_estimate, 1,
      // extend to allow for vector of prior sds mu_prior_sd_global,
      mu_n_sigma, k,
      mu_n_sigma_fixed,
      mu_n_sigma_estimate);
  matrix[C,k] mu = get_morehierparam(mu_sigma, mu_raw_fixed, mu_raw_estimate,
  // make ordering comparable to oneparam
  k, mu_n_terms_fixed, mu_n_terms_estimate,
      mu_re_start, mu_re_end, C, mu_n_terms,
...
model {
  ...
  // need to rowvector or something like that?
  to_vector(mu_raw_estimate) ~ std_normal();
  to_vector(mu_sigma_estimate) ~ normal(0, 1)T[0, positive_infinity()];
  // data model parameters
  nonse_estimate ~ normal(0, 0.5)T[0, positive_infinity()];

  // fit to the data
  for(i in 1:N) {
    y[i] ~ normal(mu[geo_unit[i],1], nonse);
    y[i] ~ normal(mu[geo_unit[i],2], nonse);
  }
}
...
generated quantities {
  // make ordering comparable to oneparam
  // remove these uvw stuff
  matrix[mu_n_terms,k] mu_star = get_morehierparam_star(mu_sigma, mu_raw_fixed, mu_raw_estimate,
      k, mu_n_terms_fixed, mu_n_terms_estimate,
      mu_re_start, mu_re_end, C, mu_n_terms,
      mu_model_matrix_w, mu_model_matrix_v, mu_model_matrix_u);
