...
-   We clean up helper functions, so that they take in the mu_stars, and specific other stan_data, and are better documented. See function extract_parameter_subhierarchical in fit_model_simplified R script, and its usage below.
...
```{r}
#get_posterior_summaries_simplified(fit1a) 
fit <- fit1a
...
get results for 1 parameter name

```{r}

get_posterior_summary_one_fixable_param(prefixed_param_name = fixable_params_to_collect[1], fit = fit)
```

when using a fit where things are fixed, fixed values and estimates are combined

(this will be in test case for subnational, we could also add something like 1a and 1b)

```{r}
#prefixed_param_name
```

how this summary is used... global fit is used when creaiting the stan data

hier data does not depend on fixed values
...
use for non-hierarchical parameters, sigma_y here (vector for the full fpet model)
