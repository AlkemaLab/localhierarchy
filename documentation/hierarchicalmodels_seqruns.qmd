...
The set up and notation in stan/R, in terms of parameters, using parameter Omega as an example, is currently as follows:

-   Omega_raw refers to the vector with ALL $\gamma$s, including the global one.The ordering is global-allregional-allsubregional-country.

-   Omega_sigma referred to the vector of stacked sigmas, excluding $s_{\mu, global}$ (in the original package, it was called Omega_sigma_raw).

-   Omega_star refers to the vector with (mu global and ALL $\eta$s). we get Omega_stars from Omega_raw and the vector ($s_{\mu, global}$ , Omega_sigma), using the stan `scale_blocks` function.

-   Omega refers to the final $\mu$s at the country level. Matrix multiplication is used to obtain the Omega country vector from the Omega_stars. The full matrix `Omega_model_matrix` is a sparse matrix, with 1s and 0s (columns refer to $\mu_{global}$, the regional etas, the subregional etas, the country etas). The matrix multiplication is implemented using the stan function csr_matrix_times_vector, using a decomposition of `Omega_model_matrix` into different parts U, V, W.

-   Because of the sequential fitting, for Omega_raw and Omega_sigma, we also had to distinguish between parameters that were fixed vs those that were estimated. We do so with the \_fixed and \_estimate additions, i.e. the vector of Omega_raws is produced from Omega_raw_fixed and Omega_raw_estimate.
...
in the simpler model used, various Omegas are defined in the transformed parameters block so we can check those directly. (Note that this not (yet) true for the transition model because the creation of parameters takes place within the transition function, see notes in last section on this).

here just checking the sigmas and first Omega:

```{r}

fit <- fit1a
 parname_list <- list( c("Omega_raw_estimate[1]",
...
-   To deal with the set up where things are fixed vs estimated, we let the stan model spit out the Omega_stars, that's what the original helper functions use as inputs.

-   We clean up helper functions, so that they take in the Omega_stars, and specific other stan_data, and are better documented. See function extract_parameter_subhierarchical_simplified in fit_model_simplified R script, and its usage below.
...
# Omega[[]] has posterior samples for the subhierarchy level
...
res_global <- map(Omega, function(tibble_samples) 
...
res_local <- map(Omega, function(tibble_samples) 
  tibble_samples %>% select(name, value)  %>% reframe(quantile_df(value), .by = name))
res_local
```
...
-   How to get the Omegas in full model set up: in the simpler model used here, various Omegas are defined in the transformed parameters block so we can check those directly. Note that this not (yet) true for the full transition model because the creation of parameters takes place within the process model function. Options to obtain the Omegas in the full model are to

    1.  add them to generated quantities: pull out omegas with an `if {add_output_omegas}` type statement, re-using functions/statements introduced in the simpler file here.

    2.  rewrite transformed parameter block and pull Omegas out of the process model function (not sure if that slows things down when not used).
