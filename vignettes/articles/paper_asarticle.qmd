...
<!-- % introduce local models and how they compare to global model, why they are needed -->
...
To motivate the utility and use of the \texttt{localhierarchy} package, we focus on the Family Planning Estimation Tool (FPET) as an illustrative example. FPET is widely used to estimate key family planning indicators in countries and regions. Key indicators include modern contraceptive use, demand for family planning, and demand satisfied, referring to the ratio of modern contraceptive use to demand. Data on such indicators is available from household surveys and routine data systems. FPET estimates are obtained from fitting a Bayesian model to available data. An illustration of FPET estimates and available data is shown in @fig-localfits for three countries in Western Africa.
...
```{=html}
<!-- Here, if $\mu^{(\mathrm{global})}$ has prior $\mu^{(\mathrm{global})} \sim N(0, s_{\mu, global}^2)$, then $$\mu_c =  s_{global}\cdot \gamma_{global} + \sigma_{ region}\cdot \gamma_{r[s[c]]]}^{(
egion)} + \sigma_{ subregion}\cdot\gamma_{s[c]}^{(\mathrm{subregion})} + \sigma_{ country}\cdot\gamma_{ c},$$ with $\gamma \sim N(0,1)$ for each gamma (one global, $R$ regions, $S$ subregions, $C$ countries). -->
```
...
The package includes a user-friendly specification of hierarchical model structures, and associated specification of fixed parameters for local runs. A worked example is given in the first use case (@sec-nationalusecase). Briefly, if a user wants to specify a hierarchical model with the levels world, regions, subregion, and country, <!-- country parameters $\mu_c^{(\mathrm{country})} =  s_{global}\cdot \gamma_{global} + \sigma_{ region}\cdot \gamma_{r[s[c]]]}^{(
...
# Use case 1: National estimation {#sec-nationalusecase}
...
<!-- \node[level0] (mu0) {$\mu^{(\mathrm{global})} \sim \mathcal{N}(0,s_\mathrm{global}^2)$}; -->

<!-- % --- Level 1 --- -->

<!-- \node[level1, below left=1.5cm and 3cm of mu0] (mu11) {$\mu^{(\mathrm{country})}_1 \sim \mathcal{N}(\mu^{(\mathrm{global})},\sigma_{\mathrm{country}}^2)$}; -->

<!-- \node[level1, right=of mu11] (mu12) {$\mu^{(\mathrm{country})}_2$}; -->

<!-- \node[level1, right=of mu12] (dots1) {$\dots$}; -->

<!-- % --- Level 2 --- -->

<!-- \node[level2, below left=1.5cm and 0cm of mu11] (mu21) {$\mu^{(\mathrm{subnat})}_1 \sim \mathcal{N}(\mu^{(\mathrm{country})}_1,\sigma_{\mathrm{subnat}}^2)$}; -->
...
<!-- \node[anchor=north west] at (bg1.east) {$\sigma_{\mathrm{country}}$, country}; -->

<!-- \node[anchor=north west] at (bg2.east) {$\sigma_{\mathrm{subnat}}$, subnational region}; -->
...
<!-- \node[level1] (mu0) {$\mu^{(\mathrm{country})}_1$}; -->

<!-- % --- Level 1 --- -->

<!-- \node[data, below left=1.5cm and 3cm of mu0] (mu11) {$y^{(country)}_1 \sim \mathcal{N}(\mu^{(\mathrm{country})}_1,\sigma_{y}^2)$}; -->
...
<!-- \node[level2] (mu0) {$\mu^{(\mathrm{subnat})}_1$}; -->

<!-- % --- Level 1 --- -->

<!-- \node[data, below left=1.5cm and 3cm of mu0] (mu11) {$y^{(subnat)}_1 \sim \mathcal{N}(\mu^{(\mathrm{subnat})}_1,\sigma_{y}^2)$}; -->
...
<!-- \mu_w^{(\mathrm{region})} &\sim N(\mu^{(\mathrm{global})}, \sigma_{ \mathrm{region}}^2),\\ -->

<!-- \mu_s^{(\mathrm{subregion})} &\sim N(\mu_{w[s]}^{(\mathrm{region})}, \sigma_{ \mathrm{subregion}}^2), \\ -->

<!-- \mu_c^{(\mathrm{country})} &\sim N(\mu_{s[c]}^{(\mathrm{subregion})}, \sigma_{ \mathrm{country}}^2),  \\ -->

<!-- \mu_r^{(\mathrm{subnat})} &\sim N(\mu_{c[r]}^{(\mathrm{country})}, \sigma_{ \mathrm{subnat}}^2),  \\ -->
...
The simulation setup for subnational parameters and data is summarized in Figure \ref{fig-simusetup} in three panels. As explained above for national estimation, the top panel indicates the hierarchical model considered. For subnational estimation, an additional level is added, with $\mu_r^{(\mathrm{subnat})} = \mu_{c[r]}^{(\mathrm{country})} + \eta_{r}^{(\mathrm{subnat})}$, $\eta_{r}^{(\mathrm{subnat})}|\sigma_{ \mathrm{subnat}} \sim N(0, \sigma_{ \mathrm{subnat}})$.
...
Simulated data at the subnational level was generated using the `simulate_multilevel_data` function described above. For subnational levels, we set $\sigma_{\mathrm{subnat}} = 0.2$, and we created 20 subnational regions in each country and 10 observations per region. In the simulated data, `level1` refers to the country level and `level2` refers to the subnational level. The data sets contain the simulated parameters (`eta`) and data (`y`):
...
The R package is available at [github.com/AlkemaLab/localhierarchy](https://github.com/AlkemaLab/localhierarchy). <!-- and Zenodo \[to do: add reference\]. --> This code is distributed under the BSD 3-Clause License. See the R package's LICENSE file for details.
